...
Commentaires:
- Revoir EVENT_TYPES pour faciliter l'ajout de nouveaux types d'événements ?
"""
...
class EventsRegister:
    ...
    def load(self) -> None:
        ...
        try:
            with open(self.event_file, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    try:
                        ...
                        logging.error("Compteur invalide pour événement: %s", line)
                        continue
            ...
            logging.info("Registre d'événements chargé: %d événements", len(self.event_list))
            ...
            logging.error("Erreur lors du chargement du registre: %s", e)
            self.loaded = False
    ...
    def add(self, location: str, sample: str, event_type: str) -> None:
        ...
        if event_type not in self._valid_event_types:
            logging.error(
                "Type d'événement inconnu '%s'. Types valides: %s",
                event_type, ', '.join(sorted(self._valid_event_types))
            )
            self.registration_fail = True
            return
    ...
    def save(self) -> None:
        ...
        try:
            ...
            logging.debug("Registre sauvegardé: %s", self.event_file)
        except (IOError, OSError) as e:

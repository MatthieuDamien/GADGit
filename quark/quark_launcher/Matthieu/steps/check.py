...
class CheckStep(BaseStep):
    ...
    def _check_analysis(self, analysis_dir: str, family: str) -> None:
        ...
        """
        logging.info("Vérification de l'analyse: %s", family)
        ...
        analysis_ended, _ = self._check_analysis_status(
    ...
    def _check_dispatch(self, analysis_dir: str, family: str) -> list:
        ...
        if dispatched:
            logging.info("%s: %d échantillon(s) dispatché(s) - %s",
                         family, len(dispatched), ', '.join(dispatched))
            ...
            logging.error("%s: aucun échantillon trouvé dans %s. "
                          "Vérifier si dispatch en cours.", family, analysis_dir)
    ...
    def _check_analysis_status(
        ...
        if not file_exists(status_file, log=False):
            logging.info("%s: analyse non terminée (pas de status.tsv)", family)
            self.events.add(analysis_dir, family, "analysis_not_ended")
            return False, False

        # Analyse terminée
        logging.info("%s: analyse terminée (status.tsv trouvé)", family)
        ...
        if status_info['has_failures']:
            logging.warning("%s: échec(s) détecté(s) - %s",
                            family, ', '.join(status_info['failed_processes']))
            ...
            # Analyse réussie
            logging.info("%s: analyse réussie", family)
    ...
    def _check_archiving(
        ...
        if not file_exists(archive_log, log=False):
            ...
            logging.info("%s: pas de fichier archive.log", family)
            return
        ...
        try:
            ...
            logging.error("Erreur lecture %s: %s", archive_log, e)
            return
        ...
        if end_success:
            logging.info("%s: archivage terminé avec succès", family)
            ...
            logging.error("%s: archivage arrêté prématurément", family)
            ...
            logging.info("%s: archivage en cours", family)
    ...
    def _check_event_counts(self) -> None:
        ...
        for event_str, count in all_events:
            ...
            if event_type == "analysis_not_ended" and count > 0 and count % self.EVENT_COUNT_LIMIT_STUCK == 0:
                warning_msg = (
            ...
            # Alertes pour autres événements problématiques
            elif count > 0 and count % self.EVENT_COUNT_LIMIT == 0:
                ...
                if event_type not in success_events + ["analysis_not_ended"]:
                    warning_msg = (
